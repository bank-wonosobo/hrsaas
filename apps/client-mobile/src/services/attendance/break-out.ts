...
export const breakOutService = async (
  payload: ClockInReq,
  photo: PhotoResult,
  ...
  const response = await api.post("/attendances/break-out", formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });

  if (response.status !== 200 && response.status !== 201) {
    throw new Error(
      response.data?.error || response.data?.message || "Gagal melakukan break-out",
    );
  }
